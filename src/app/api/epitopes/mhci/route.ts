...
import { step4MHCI } from '@/lib/step5-7-epitopes';
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[mhci] MHC-I: canonical=${canonicalSeq.length}aa mutated=${mutatedSeq.length}aa`);

    const onProgress = (current: number, total: number, message: string) => {
      if (jobId) setProgress(jobId, current, total, message);
    };

    const result = await step4MHCI(geneName, canonicalSeq, mutatedSeq, onProgress);

    if (jobId) setProgress(jobId, 1, 1, 'Done', true);

    console.log(`[mhci] canonical=${result.canonical.rows.length} rows, mutated=${result.mutated.rows.length} rows`);
    ...
    console.error('[mhci] Error:', error);
