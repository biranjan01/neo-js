...
import { step6BCell } from '@/lib/step5-7-epitopes';
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[bcell] B-cell: canonical=${canonicalSeq.length}aa mutated=${mutatedSeq.length}aa`);

    const onProgress = (current: number, total: number, message: string) => {
      if (jobId) setProgress(jobId, current, total, message);
    };

    const result = await step6BCell(geneName, canonicalSeq, mutatedSeq, onProgress);

    if (jobId) setProgress(jobId, 1, 1, 'Done', true);

    console.log(`[bcell] canonical=${result.canonical.rows.length} rows, mutated=${result.mutated.rows.length} rows`);
    ...
    console.error('[bcell] Error:', error);
